...
func GetPVByNameViaRDE(pvName string, testingClient *testingsdk.TestClient, resourceType string) (bool, error) {
	resourceSet, err := testingsdk.GetVCDResourceSet(context.TODO(), testingClient.VcdClient, testingClient.ClusterId, vcdsdk.ComponentCSI)
	if err != nil {
		return false, fmt.Errorf("error occurred while getting resource set in cluster [%s]", testingClient.ClusterId)
	}
	...
	if len(updatedVcdResourceSet) == 1 {
		...
		return false, fmt.Errorf("get more than one PV [%s] in cluster [%s]", pvName, testingClient.ClusterId)
	}
